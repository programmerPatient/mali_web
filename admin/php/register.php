...
$result = $mysql->fetch_one("SELECT * FROM admin WHERE account="."'".$_POST['account']."'");
...
if($error != ''){
	header('location:../init_admin.php?error='.$error);
}else{
	$data['name'] = $_POST['name'];
	$data['phone'] = $_POST['phone'];
	$data['account'] = $_POST['account'];
	$data['password'] = md5($_POST['password']);
	$result = $mysql->insert($data,'admin');
	$_SESSION['adminname'] =$_POST['name'];
	$_SESSION['adminphone'] = $_POST['phone'];
	$_SESSION['adminaccount'] = $_POST['account'];
